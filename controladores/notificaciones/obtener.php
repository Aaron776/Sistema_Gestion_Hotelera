...
try {
    // 1. Contar no leídas
    $sql_count = $conexion->prepare("SELECT COUNT(*) FROM notificaciones WHERE usuario_id = :uid AND leida = false");
    $sql_count->execute([':uid' => $usuario_id]);
    $no_leidas = $sql_count->fetchColumn();

    // 2. Obtener las últimas 5 notificaciones
    $sql_list = $conexion->prepare("SELECT id, mensaje, leida, fecha 
                                    FROM notificaciones 
